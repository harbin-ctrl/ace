...
/* TINE is a guest application in ACE.  Commands entered through DO and FB
   must therefore go back through the ACE AmigaDOS shell, not through the
   host's /bin/sh.  ACE publishes the shell executable in the environment of
   every command it starts. */
static int
make_amiga_script(const char *command, char *name, size_t name_size)
{
   char template_name[] = "T:TINE-XXXXXX";
   int fd = ace_amiga_posix_mkstemp(template_name);

   if (fd < 0)
      return -1;
   if (strlen(template_name) >= name_size ||
       !safewrite(fd, command, strlen(command)) ||
       !safewrite(fd, "\n", 1)){
      int saved_error = errno ? errno : EIO;
      close(fd);
      (void)ace_amiga_posix_unlink(template_name);
      errno = saved_error;
      return -1;
   }
   close(fd);
...
static int
run_amiga_script(const char *name, int input_fd, int output_fd)
{
   const char *shell = getenv("ACE_USER_SHELL");
   int status;
   pid_t child;
   ...
   if (child == 0){
      ...
      if (output_fd >= 0 && dup2(output_fd, STDOUT_FILENO) < 0)
         _exit(EXIT_FAILURE);
      if (output_fd >= 0 && dup2(output_fd, STDERR_FILENO) < 0)
         _exit(EXIT_FAILURE);
      (void)unsetenv("ACE_SCRIPT_INPUT");
      (void)unsetenv("ACE_COMMAND_ARGUMENTS");
      if (setenv("ACE_STARTUP_SCRIPT", name, 1) != 0)
         _exit(EXIT_FAILURE);
...
COMMAND(do, NOLOCATOR) /* do a shell command */
   ...
   if (!s)
      ERROR("Out of memory");
   if (make_amiga_script(s, script, sizeof(script)) != 0){
      free(s);
      ERROR("Could not create command script");
   }
   endwin();
   (void)run_amiga_script(script, -1, -1);
   (void)ace_amiga_posix_unlink(script);
...
COMMAND(fb, MARK | NEEDSBLOCK | CLEARSBLOCK) /* filter block through command */
   POS wp = pos(v->bs, 0);
   char tfn[] = "T:TINE-XXXXXX";
   int tfd = ace_amiga_posix_mkstemp(tfn);
   if (tfd < 0)
      ERROR("Could not open temporary file");

   char ifn[] = "T:TINE-XXXXXX";
   int ifd = ace_amiga_posix_mkstemp(ifn);
   if (ifd < 0){
      close(tfd);
      (void)ace_amiga_posix_unlink(tfn);
      ERROR("Could not open temporary file");
   }
   ...
   if (!s){
      close(tfd); close(ifd);
      (void)ace_amiga_posix_unlink(tfn);
      (void)ace_amiga_posix_unlink(ifn);
      ERROR("Out of memory");
   }
   ...
   free(s);
   writelines(ifd, e, v->b, v->bs, v->be);
   ifd = ace_amiga_posix_open(ifn, O_RDONLY);
   if (ifd >= 0){
      (void)run_amiga_script(script, ifd, tfd);
      close(ifd);
   }
   (void)ace_amiga_posix_unlink(script);
   (void)ace_amiga_posix_unlink(ifn);
   ...
   if (!ws){
      free(ws); (void)ace_amiga_posix_unlink(tfn);
      ERROR("Out of memory");
   }
   v->p = wp;
...
COMMAND(sa, NOLOCATOR) /* save text to file */
    ...
    int fd = ace_amiga_posix_open(fn, O_WRONLY | O_CREAT | O_TRUNC, 0644);
    if (fd < 0){
       free(fn);
...
COMMAND(sh, NOLOCATOR) /* show information */
    WINDOW *w = e->docview.w;
    WINDOW *c = e->cmdview.w;
    char display_name[FILENAME_MAX + 1];
    char display_dir[FILENAME_MAX + 1];
    ...
    /* basename() and dirname() may modify their arguments.  Never hand
       either of them EDITOR::name: an AmigaDOS name such as SYS:file would
       otherwise be damaged merely by asking ED for its status. */
    snprintf(display_name, sizeof(display_name), "%s", e->name);
    snprintf(display_dir, sizeof(display_dir), "%s", e->name);

    wattron(w, A_BOLD);
    werase(w);
    mvwprintw(w, 0, 0, "Editing file    %s",     basename(display_name));
    mvwprintw(w, 1, 0, "In directory    %-.61s", dirname(display_dir));
