#ifndef ACE_PRIVILEGE_CLIENT_H
#define ACE_PRIVILEGE_CLIENT_H
...
/*
 * The broker's side of the fmm channel.
 *
...
/*
 * Launch the fmm and complete the handshake.
 *
 ...
 * and not fatal: an ACE session whose fmm will not start is an ordinary
 ...
 * fmm unelevated as itself.  It is a parameter rather than an
 ...
 * against it.
 *
 * program overrides which executable is launched, or NULL for the fmm
...
/* What the fmm actually granted, which may be less than was asked. */
...
/* The fmm's pid.  Clients setns() to its mount namespace once the volume
...
/* Round-trip liveness check.  Returns 0 if the fmm answered. */
int ace_privilege_connection_ping(struct ace_privilege_connection *fmm);

/*
 * Ask the fmm to give up privilege and exit, at the user's request,
 * without ending the ACE session.  The handle is spent afterwards and must
 * still be freed with ace_privilege_connection_close().
 */
...
/*
 * Obtain the CRM's channel.
 *
 * A second, independent conversation with a second process: one that lives
 * inside the volume fmm's mount namespace and can therefore open the
 ...
 *
 * The returned handle is closed with ace_privilege_connection_close() like any other, and
...
/*
 * Mount one discovered device, and learn where the fmm put it.
 *
 ...
 * no parameter through which it could: the fmm derives /dev/<name>,
...
/*
 * One typed operation on one exact object, addressed to the CRM.
 *
 * path is relative and never begins with a slash: which domain it is in comes
 * from flags (ACE_PRIVILEGE_FLAG_HOST_PATH for an absolute host object, clear
